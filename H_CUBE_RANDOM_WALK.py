...
for pp in range(len(p)):
    ...
    ID0=np.array(ID0)
    IDP[pp]=np.mean(ID0[:,2])
    DIDP[pp]=np.std(ID0[:,2])/np.sqrt(len(p))
    ID1P[pp]=np.mean(ID0[:,1])
    DID1P[pp]=np.std(ID0[:,1])/np.sqrt(len(p))

#INSERT PUBLISHER DETAILS AND INITIALIZE IT

name_var=["RWALK","L","NS","NR"]
var=[n_method,L,n_mean,snap_time]    

pub=class_WF.publisher(name_var,var,["ID","ID1"])
pub.create()
suma=0
for x in range(len(p)):
    pub.write([p[x],IDP[x],DIDP[x],ID1P[x],DID1P[x]])
