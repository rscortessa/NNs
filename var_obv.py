...
if exvar==False:
    if each==False:
        for j in range(n_mean):
            m[j,:],En[j,:],Kback[j,:],S_ent[j,:]=v_state_steady(model[n],n_sample,hi,n_run,L,Gamma,dh,NG,Hts,Eigvs,each) ;
    else:
        for j in range(n_mean):
            sisj[j,:],m[j,:],En[j,:],Kback[j,:],S_ent[j,:]=v_state_steady(model[n],n_sample,hi,n_run,L,Gamma,dh,NG,Hts,Eigvs,each) ;

else:
    if each==False:
        for j in range(n_mean):
            m[j,:],En[j,:],Kback[j,:],S_ent[j,:],vars[j,:]=v_state_steady(model[n],n_sample,hi,n_run,L,Gamma,dh,NG,Hts,Eigvs,each) ;
    else:
        for j in range(n_mean):
            sisj[j,:],m[j,:],En[j,:],Kback[j,:],S_ent[j,:],vars[j,:]=v_state_steady(model[n],n_sample,hi,n_run,L,Gamma,dh,NG,Hts,Eigvs,each) ;
...
file.write("G"+"\t"+" dE"+"\t"+" E"+"\t"+" dS"+"\t"+" S"+"\t"+" dm"+"\t"+" m"+" \t"+"dKb " +"\t "+ "Kb"+"\n")
if each!=False:
    file2=open(name+"SiSj.txt","w")

for gg in range(NG):
    Gnew=Gamma+(GammaF-Gamma)/(NG)*gg
    file.write(str(Gnew)+"\t"+str(En_error[gg])+"\t"+str(En[gg])+"\t"+str(S_error[gg])+"\t"+str(S_ent[gg])+"\t"+str(m_error[gg])+"\t"+str(m[gg])+"\t"+str(Kb_error[gg])+"\t"+str(Kb[gg])+"\n")
    if each!=False:
        for jj in range(L-1):
