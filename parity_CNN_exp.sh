...
architecture="parity_CNN_COMPLEX"    #architecture (in this case is the transformer)
kernel_size=5
features={2,10}
head_dim=$L
n_patches=5
...
for angle in "${angles[@]}"; do
    ...
    for ((niter=1; niter<=Nrep; niter++)); do
        ...
        python PLAYING_WITH_NN_RUNNING.py --L "$L" --g "$g" --angle "$angle" --architecture "$architecture" --features 10 10 --kernel_size "$kernel_size" --padding "SAME"   &
