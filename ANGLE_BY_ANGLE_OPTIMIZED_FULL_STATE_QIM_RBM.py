...
for g in G:
    ...
    if hyper_parameters_ready:
        ...
        objective_final=partial(objective,model=model,L=L*W,hi=hi,H=H,n_iter=n_iter,holomorphic=holomorphic)
    ...
    #OBSERVABLES INIT.
    obs={}
    obs["P"]=parity_IsingModel(angle[ii_angle],L,hi)
    obs["M"]=rotated_m(angle[ii_angle],L,hi)
    
    for jj in sites_corr:
        obs["CZ0"+jj]=Sz0Szj(angle[ii_angle],L,hi,int(jj))
        obs["CX0"+jj]=Sx0Sxj(angle[ii_angle],L,hi,int(jj))
    ...
    for tt in range(NMEAN):
        ...
        #LAST CALCULATION
        PSI.save_params(kk,log2)
        log2.serialize(MASTER_DIR+"/"+SLAVE_DIR+"/"+str(tt)+"NM"+str(ii_angle)+FILENAME+"VAR")
