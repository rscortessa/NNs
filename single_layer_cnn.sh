...
for angle in "${angles[@]}"; do
    for feat_1 in "${feat_a[@]}"; do
		  for kernel in "${kernel_size[@]}"; do
		      python PLAYING_WITH_NN_OPTUNA_STUDY.py --L "$L" --g "$g" --angle "$angle" --architecture "$architecture" --features "$feat_1"  --kernel_size "$kernel" --padding "SAME"  --NR "$NR" --trials "$trials"  &
		  done
    done
done
