...
while (( $(echo "$Gamma <= $Gamma_end" | bc -l) )); do
    ...
    julia DMRG_BIQISING.jl "$L" "$Gamma" "$W" "$N_samples"
    python run_MPS.py "$L" "$Gamma" "$W" "$N_samples"
    python datasetMPS_run.py "$L" "$Gamma" "$W" "$N_samples"
    python dataset_density_run.py "$L" "$Gamma" "$W" "$N_samples" "1000" "5"
