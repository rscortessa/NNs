...
architecture="factored_attention_T"    #architecture (in this case is the transformer)
n_heads=1
head_dim=$L
n_patches=2
NR=10000
...
for angle in "${angles[@]}"; do
    ...
    for ((niter=1; niter<=Nrep; niter++)); do
        ...
        python PLAYING_WITH_NN_RUNNING.py --L "$L" --g "$g" --angle "$angle" --architecture "$architecture" --n_heads "$n_heads" --head_dim "$head_dim" --n_patches "$n_patches" --clipping "True" --NR "$NR" &
