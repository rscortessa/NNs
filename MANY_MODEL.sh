...
echo "Number of repetitions =$N_rep"
echo "Model simulated = $MODEL"

# Loop over Gamma values
Gamma=$Gamma_start

dir=${MODEL}L${L}W${W}NS${N_samples}GI${Gamma_start}GF${Gamma_end}NR${N_rep}
mkdir "$dir"

while (( $(echo "$Gamma <= $Gamma_end" | bc -l) )); do
    echo "Running with Gamma=$Gamma, L=$L, N_samples=$N_samples"
    bash MULT_MODEL.sh $L $W $N_samples $Gamma $N_rep $MODEL $dir
